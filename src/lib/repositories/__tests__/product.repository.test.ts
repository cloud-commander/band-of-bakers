...
describe("ProductRepository", () => {
  ...
  beforeEach(() => {
    ...
    // Query builder mocks
    mocks.mockDb.query.products.findMany = vi.fn();
  });

  describe("findBySlug", () => {
    it("returns product when found", async () => {
      const mockProduct = { id: "1", slug: "test-product" };
      mocks.mockQueryBuilder.then.mockImplementation((resolve: any) => resolve([mockProduct]));
  ...
  describe("findActiveProducts", () => {
    it("returns active products", async () => {
      ...
      mocks.mockQueryBuilder.then.mockImplementation((resolve: any) => resolve(mockProducts));
  ...
  describe("toggleActive", () => {
    it("toggles product status", async () => {
      ...
      mocks.mockQueryBuilder.then.mockImplementation((resolve: any) => resolve([updatedProduct]));
  ...
  describe("countActive", () => {
    it("counts active products", async () => {
      mocks.mockQueryBuilder.then.mockImplementation((resolve: any) => resolve([{ count: 5 }]));
  ...
  describe("createWithVariants", () => {
    it("creates product and variants", async () => {
      ...
      mocks.mockQueryBuilder.then.mockImplementation((resolve: any) => resolve([newProduct]));
      ...
      expect(mocks.mockDb.insert).toHaveBeenCalledTimes(2); // Product + Variants
    });
    ...
    it("creates product without variants", async () => {
      ...
      expect(result).toEqual(newProduct);
      expect(mocks.mockDb.insert).toHaveBeenCalledTimes(1); // Product only
    });
  });
  ...
  describe("findPaginated", () => {
    it("returns paginated products", async () => {
      ...
      mocks.mockDb.query.products.findMany.mockResolvedValue(mockProducts);
      mocks.mockQueryBuilder.then.mockImplementation((resolve: any) => resolve([{ count: 1 }])); // For count query
    ...
    it("returns paginated active products", async () => {
      const mockProducts = [{ id: "1", is_active: true }];
      mocks.mockDb.query.products.findMany.mockResolvedValue(mockProducts);
      mocks.mockQueryBuilder.then.mockImplementation((resolve: any) => resolve([{ count: 1 }]));
  ...
  describe("getVariants", () => {
    it("returns product variants", async () => {
      ...
      mocks.mockQueryBuilder.then.mockImplementation((resolve: any) => resolve(mockVariants));
  ...
  describe("updateWithVariants", () => {
    it("updates product and variants", async () => {
      ...
      expect(result).toEqual(updatedProduct);
      expect(mocks.mockDb.update).toHaveBeenCalled(); // Product update + Variant update
      expect(mocks.mockDb.insert).toHaveBeenCalled(); // Variant create
      expect(mocks.mockDb.delete).toHaveBeenCalled(); // Variant delete
    });
