...
export default function AdminOrdersPage() {
  ...
  const getBakeSaleDate = (bakeSaleId: string) => {
    const bakeSale = mockBakeSalesWithLocation.find((bs) => bs.id === bakeSaleId);
    return bakeSale ? new Date(bakeSale.date) : null;
  };
  ...
  const filteredAndSortedOrders = useMemo(() => {
    let filtered = mockOrdersWithItems;

    // Apply search filter
    if (debouncedSearchTerm) {
      const searchLower = debouncedSearchTerm.toLowerCase();
      filtered = filtered.filter((order) => {
        const customerName = getCustomerName(order.user_id).toLowerCase();
        const bakeSale = mockBakeSalesWithLocation.find((bs) => bs.id === order.bake_sale_id);
        const bakeSaleDate = bakeSale ? new Date(bakeSale.date).toLocaleDateString("en-GB") : "";
    ...
    return filtered.sort((a, b) => {
      ...
      switch (sortField) {
        case "created_at":
          ...
          break;
        case "bake_sale_date":
          aValue = getBakeSaleDate(a.bake_sale_id)?.getTime() || 0;
          bValue = getBakeSaleDate(b.bake_sale_id)?.getTime() || 0;
          break;
        case "total":
          aValue = a.total;
          bValue = b.total;
  ...
  const getQuickAction = (order: (typeof mockOrdersWithItems)[0]) => {
          ...
          <tbody>
            {paginatedOrders.map((order) => (
              <tr key={order.id} className="border-t hover:bg-muted/30 transition-colors">
                ...
                <td className="p-4 text-sm text-muted-foreground">
                  {getBakeSaleDate(order.bake_sale_id)?.toLocaleDateString("en-GB", {
                    year: "numeric",
                    month: "short",
                    day: "numeric",
                  }) || "N/A"}
                </td>
                <td className="p-4 text-sm font-medium">{getCustomerName(order.user_id)}</td>
                <td className="p-4">
                  <Badge
                    variant="outline"
                    className={cn(
